  ...
  class V1CreateSubOrganizationIntentV3
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TurnkeyClient::V1CreateSubOrganizationIntentV3` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TurnkeyClient::V1CreateSubOrganizationIntentV3`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @private_keys.nil?
        invalid_properties.push('invalid value for "private_keys", private_keys cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @private_keys.nil?
      true
    end
    ...
    def hash
      [sub_organization_name, root_users, root_quorum_threshold, private_keys].hash
    end
