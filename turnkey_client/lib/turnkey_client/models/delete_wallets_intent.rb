  ...
  class DeleteWalletsIntent
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TurnkeyClient::DeleteWalletsIntent` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TurnkeyClient::DeleteWalletsIntent`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'wallet_ids')
        if (value = attributes[:'wallet_ids']).is_a?(Array)
          self.wallet_ids = value
        end
      end
    ...
    def list_invalid_properties
      ...
      if @wallet_ids.nil?
        invalid_properties.push('invalid value for "wallet_ids", wallet_ids cannot be nil.')
      end
    ...
    def valid?
      return false if @wallet_ids.nil?
      true
    end
    ...
    def hash
      [wallet_ids, delete_without_export].hash
    end
