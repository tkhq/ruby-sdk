  ...
  class DeploymentStatus
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TurnkeyClient::DeploymentStatus` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TurnkeyClient::DeploymentStatus`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @ready_replicas.nil?
        invalid_properties.push('invalid value for "ready_replicas", ready_replicas cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @ready_replicas.nil?
    ...
    def hash
      [deployment_id, ready_replicas, desired_replicas, last_updated_time].hash
    end
