  ...
  class FiatOnRampCredential
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TurnkeyClient::FiatOnRampCredential` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TurnkeyClient::FiatOnRampCredential`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [fiat_onramp_credential_id, organization_id, onramp_provider, project_id, publishable_api_key, encrypted_secret_api_key, encrypted_private_api_key, sandbox_mode, created_at, updated_at].hash
    end
